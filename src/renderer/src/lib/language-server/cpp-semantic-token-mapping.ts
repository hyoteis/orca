...
const FALLBACK_TOKEN_TYPE = CPP_SEMANTIC_TOKEN_TYPES.indexOf('variable')
...
export function remapCppSemanticTokenData(
  ...
  const tokenTypeIndexes = serverLegend.tokenTypes.map((type) => {
    ...
    return index === -1 ? FALLBACK_TOKEN_TYPE : index
  })
  ...
  for (let index = 0; index + 4 < result.length; index += 5) {
    result[index + 3] = tokenTypeIndexes[result[index + 3]] ?? FALLBACK_TOKEN_TYPE
