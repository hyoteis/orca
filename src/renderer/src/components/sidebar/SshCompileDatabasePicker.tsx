...
import { ArrowUp, FileJson, Folder, Loader2, X } from 'lucide-react'
 ...
 * browses the remote host"). Directories navigate, JSON files pick. */
export function SshCompileDatabasePicker({
  ...
  const databases = entries.filter((entry) => !entry.isDirectory && entry.name.endsWith('.json'))
